...
`web_listening` workflow and then ingested by this repository. The repository
now owns the strict prompt, authoring contract, and production-capable CLI path;
live Hermes command changes, scheduler state, credentials, and cutover remain
separate host-runtime work. See
[`ownership-runbook.md`](../monitoring/jobs/weekly-climate-monitor-08h/docs/ownership-runbook.md).
