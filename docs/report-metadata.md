...
object used by the repository-owned weekly authoring contract. Schema-only
validation is insufficient: the Python
...
web_listening acquisition (existing per-site HTTP/browser policy)
  + Pillar B discovery
  -> canonical URL/history selection and frozen evidence
  -> independent Hermes invocation per URL: climate AND actuarial/insurance
     relevance, plus the atomic summary + categories + keywords bundle
  -> validate and checkpoint each result; resume unfinished URLs
  -> one executive invocation over all qualifying summaries
  -> finalize the canonical weekly Markdown and bound artifacts
  -> 09:00 summary/PDF and later Registry import
```

The relevance rules are maintained separately but included in the same per-URL
request as the semantic fields. There is no report article-count cap. The CLI
owns the queue; the deterministic finalizer validates and renders the completed
response without fetching again, making another model call, or inventing fields.
See [the current flow](../PIPELINE_REFERENCE.md).
...
- The sidecar contains every finally selected article, with no count slice.
  The production v2 path uses the validated relevance decisions and semantic
  bundles from independent URL authoring; it does not apply the old keyword
  classifier again.
- The sidecar producer makes no model call. Validated supplied semantics retain
  `semantics_provenance: "agent_bundle"`. The supported legacy path can compose
  already-computed classifier values as `semantics_provenance: "pipeline_derived"`;
  that compatibility behavior is not a second production authoring step.
...
- Articles are ordered exactly as `render_report` emits them (website, then
  document, then research for the legacy format) and correspond 1:1 with the
  rendered articles and their source links, including weekly Pillar A/B links:
  nothing missing, nothing duplicated, nothing stale.
...
New report generation writes the validated semantic metadata immediately after
each article summary. The production v2 response supplies the categories and
keywords under the pinned taxonomy; classifier-derived fields remain a legacy
compatibility path.
...
The repository owns the prepare/serial-authoring/finalize CLI and contracts;
`web_listening` owns acquisition and Hermes supplies model execution and host
scheduling. Implemented code is not proof of production cutover. See the dated
status and deployment gates in [PIPELINE_REFERENCE.md](../PIPELINE_REFERENCE.md).
